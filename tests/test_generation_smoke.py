...
def test_the_template_does_not_tell_an_instance_to_import_this_suite(generated):
    ...
    for path in ("tests/test_generation_smoke.py", "tests/faultkit.py",
                 "tests/test_fault_injection.py", ".github/workflows/smoke.yml"):
        assert not drift_check.inherited(path), (
            "the drift checklist tells an instance to import %s, which cannot "
            "run green there" % path)

    # ...and the portable half MUST travel, or row 60's exclusion is just a
    # deletion. This is the pair that makes the split honest: the generation
    # suite stays home, the invariants go out.
    for path in ("tests/test_invariants.py", "tests/requirements.txt",
                 ".github/workflows/invariants.yml"):
        assert drift_check.inherited(path), (
            "%s is not inherited, so instances never receive the guards that "
            "DO work in them - which is how the shakedown ran eight actions "
            "on mutable tags with a write-scoped PAT for weeks" % path)
