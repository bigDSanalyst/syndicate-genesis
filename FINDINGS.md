...
| 52 | **the protocol promised an exit it cannot reach for its own target users.** arXiv requires an endorsement for a first submission in a category; institutional affiliation auto-endorses, so the gate falls hardest on the unaffiliated independents the README courts. A syndicate may hold zero endorsements and no internal approval substitutes. `archive_targets` listed the gated route first; no manifest field tracked eligibility; ORCID shipped as a placeholder | Zenodo first (`[zenodo, arxiv]`), the constraint stated at Activation rather than discovered at submission, ORCID required for the publication path but never for membership, and `deposit_zenodo.py` spec'd as the tool that makes the ungated route real | docs/PUBLICATION.md - spec open |
