...
| 51 | **arXiv refuses the GitHub Actions runner range: HTTP 406 on every request.** Four days of 429 escalated to a hard refusal. Proved by probe, not inference - seven header combinations including no headers at all and a browser-like User-Agent all returned 406, so the request shape is not the cause and no client-side change fixes it | ingestion needs a different egress: run it off Actions (local or Colab, push the results), or move to OAI-PMH / bulk access. The retry and message fixes stand and are correct; they were not the blocker | open - needs an egress decision |
