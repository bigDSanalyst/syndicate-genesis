...
| 51 | **arXiv returns HTTP 406 to GitHub Actions' Azure egress.** Four days of 429 escalated to a hard refusal. Ruled out by probe, not inference: request shape (7 header combinations incl. none at all and a browser-like UA, all 406), per-repo traffic history (a second repo that has never successfully called arXiv gets the same 406 from IP 52.161.82.98), and per-IP throttling (5+ runs across rotating runner IPs, consistently 406). **Not yet ruled out: that any non-Azure egress succeeds** - one run of the same code from a non-GitHub IP settles it | run ingestion off Actions (Colab or local, push the notes), or move to OAI-PMH / bulk access. Cooling off is NOT indicated: a repo with no arXiv history is refused identically. Retry and message fixes stand; neither was the blocker | open - pending the non-Azure egress test |
